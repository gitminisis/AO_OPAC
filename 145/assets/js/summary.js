$(document).ready(function(){
    ...
    if (page !== null) {
        ...
        // Summary Bookmark
        $('#web_sum_form').on('submit', function(e) { e.preventDefault()} ); 
...
const setPaginationAccessiblity = ( page ) => {
    ...
    dblright.forEach( elem => {
        ...
        parent.nodeName == "A" ? elem.parentNode.setAttribute('aria-label', 'Next Page') : elem.setAttribute('aria-label', 'Next Page')
    })
    dblLeft.forEach( elem => {
        let parent = elem.parentNode
        parent.nodeName == "A" ? elem.parentNode.setAttribute('aria-label', 'Previous Page') : elem.setAttribute('aria-label', 'Previous Page')
    })
    prev.forEach( elem => {
        let parent = elem.parentNode
        parent.nodeName == "A" ? elem.parentNode.setAttribute('aria-label', 'Previous Page') : elem.setAttribute('aria-label', 'Previous Page')
    ...
    list.forEach( elem => {
        let parent = elem.parentNode
        parent.nodeName == "A" ? elem.parentNode.setAttribute('aria-label', 'Switch to List View') : elem.setAttribute('aria-label', 'Switch to List View')
    })
    grid.forEach( elem => {
        let parent = elem.parentNode
        parent.nodeName == "A" ? elem.parentNode.setAttribute('aria-label', 'Switch to Grid View') : elem.setAttribute('aria-label', 'Switch to Grid View')
    })
}
