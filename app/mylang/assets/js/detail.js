...
const disableRequestBtn = (url) => {
  ...
  if ( patrion_id != '' ) {
    let url1 = `https://test.aims.archives.gov.on.ca/scripts/mwimain.dll/144/DOC_REQUEST/CHECK_REQUEST_STATUS/REQ_PATRON_ID%20${patron_id}?SESSIONSEARCH#`;  
...
$(document).ready(function() {
  ...
  let countMarcBtn = 1, countMarcXML = 1;
    ...
    if (xml) {
      ...
      if(itemDetailLength > 0){
        ...
        for (var i=0; i < itemDetailLength; ++i) {
          ...
          //   : "Unavailable")  + "</form></td>";
          itemTable += "<td>" + (itemDetailJson[i].barcode != null ? `<button id='holdings_record_${itemDetailJson[i].barcode}' class='holdings_req_btn_${i+1}' onclick='biblioSubjGenerator(sessid, "${itemDetailJson[i].barcode}")'>Inquiry</button>` 
            : "Unavailable")  + "</td>";
...
const copyPermalink = (e, rep) => {
  let sisn = e.value
  console.log(rep)
  let report = rep == 'desc' ? 'DESCRIPTION_WEB/WEB_DESC_DET/SISN' : rep == 'people' ? 'PEOPLE_VAL/WEB_PEOPLE_DET_OPAC/PERSON_ID' : 'ORGANIZATION_VAL/WEB_ORG_DET_OPAC/ORG_ID'
  let url = `https://test.aims.archives.gov.on.ca/scripts/mwimain.dll/144/${report} ${sisn}?SESSIONSEARCH&NOMSG=[ao_opac]/includes/error/norecordArchives.htm`;
