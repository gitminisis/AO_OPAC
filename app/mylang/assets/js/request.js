...
$(document).ready(function() {
    ...
    if(document.getElementById("direct-request-form") != null){
        ...
        $(".para1").after("<p class='para2'>[--Call Number: --]<strong>" + 'chicken' + "</strong></p><p class='para2'>[--Volume ID: --]<strong>" + volume_id + "</strong></p>");
    }
    if(document.getElementById("request-confirmation-form") != null){
        let call_number = sessionStorage.getItem("Call Number");
        let volume_id = sessionStorage.getItem("Volume ID");
        console.log('call number: ', call_number, 'vol id: ', volume_id)
        $(".refd-dd").after("<dt class='call-num-dt'>[--Call Number--]</dt><dd class='call-number-dd'>" + 'chicken' + "</dd><dt class='volume-id-dt'>[--Volume ID--]</dt><dd class='volume-id-dd'>" + volume_id + "</dd>");
