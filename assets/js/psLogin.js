...
let checkEmail = async email => {
    ...
    const res = await fetch(
        `/scripts/mwimain.dll/144/CLIENT_REGISTRATION/WEB_EMAIL_CHECK/C_EMAIL "${email}"?SESSIONSEARCH&NOMSG=[AO_INCLUDES]noEmail.xml`
    )
...
function mwiLogin(id) {
    fetch(
        '/scripts/mwimain.dll?patronlogin&application=UNION_VIEW&language=144&file=[AO_ASSETS]html%5chome.html', {
            method: 'POST',
...
function loginRedirect() {
    let lang = sessionStorage.getItem('lang');
    if (lang && lang === 'fr') {
        window.location = '/assets/html/publicSecureLoginFr.html'

    } else {
        window.location = '/assets/html/publicSecureLogin.html'
        ...
        try {
            ...
            if (userinfo) {
                ...
                checkEmail(useremail).then(res => {
                    ...
                    if (res) {
                        if (lang && lang === 'fr') {
                            window.location = `/scripts/mwimain.dll?logon&application=UNION_VIEW&language=144&file=[AO_ASSETS]html/publicSignupFr.html`
                        } else {
                            window.location = `/scripts/mwimain.dll?logon&application=UNION_VIEW&language=144&file=[AO_ASSETS]html/publicSignup.html`
                        }
